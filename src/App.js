      ...
      <div className="flex flex-col w-full max-w-screen-lg bg-white rounded-xl shadow-xl p-8">
        ...
        <div className="bg-gray-50 p-6 rounded-lg mb-8 shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <input type="text" placeholder="Expense description" value={newExpense.description} onChange={e => setNewExpense({ ...newExpense, description: e.target.value })} className="p-2 border rounded-lg" />
            <input type="number" placeholder="Amount" value={newExpense.amount} onChange={e => setNewExpense({ ...newExpense, amount: e.target.value })} className="p-2 border rounded-lg" />
            <select value={newExpense.paidBy} onChange={e => setNewExpense({ ...newExpense, paidBy: e.target.value })} className="p-2 border rounded-lg">
              <option value="">-- Select Payer --</option>
              {housemates.map(person => <option key={person} value={person}>{person}</option>)}
            </select>
          </div>
          <div className="flex flex-wrap gap-4">
            {housemates.map(person => (
              <div key={person} className="flex flex-col border p-3 rounded shadow-sm w-40">
                ...
                <select value={newExpense.participations[person]} onChange={e => setNewExpense({ ...newExpense, participations: { ...newExpense.participations, [person]: e.target.value } })} className="w-full p-1 border rounded text-sm">
          ...
          <button onClick={addExpense} className="mt-4 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition">Add Expense</button>
        </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {housemates.map(person => (
                    <div key={person} className="border p-3 rounded w-40 shadow-sm flex flex-col text-sm">
                      ...
                      <select value={expense.participations[person]} onChange={e => updateParticipation(expense.id, person, e.target.value)} className="border rounded p-1 mb-1">
                        <option value="out">🚫 Out</option>
                        <option value="choose">📝 Choose</option>
                        <option value="use">🍽 Use</option>
                        <option value="gift">🎁 Gift</option>
                      ...
                      {expense.participations[person] !== 'out' && (
                        <>
                          <input type="number" placeholder="Custom €" value={expense.customAmounts?.[person] || ''} onChange={e => updateCustomAmount(expense.id, person, e.target.value)} className="border rounded p-1 text-xs mb-1" />
